...
def check_if_attribute_has_value(group, attribute, value, cut_from_idx=True):
    relevant_activity_idxs = np.where(group[attribute] == value)[0]
    if len(relevant_activity_idxs) > 0:
        idx = relevant_activity_idxs[0]
        group[label_col] = pos_label
        if cut_from_idx:
...
dt_labeled.to_csv(os.path.join(output_data_folder, "hospital_billing.csv"), sep=";", index=False)
del dt_labeled
