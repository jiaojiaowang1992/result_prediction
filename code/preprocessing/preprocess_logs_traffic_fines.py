...
filenames = ["Road_Traffic_Fine.csv"]
...
def is_paid_in_full(group, cut_from_idx=True):
    is_paid = group.amount.max() <= group.totalPaymentAmount.max()
    
    if is_paid:
        group[label_col] = pos_label
        if cut_from_idx:
            relevant_activity_idxs = np.where(group.totalPaymentAmount >= group.amount.max())[0]
            idx = relevant_activity_idxs[0]
            return group[:idx]
        else:
            return group
...
for filename in filenames:
    ...
    dt_labeled = data.groupby(case_id_col).apply(is_paid_in_full, cut_from_idx=True)
    dt_labeled.to_csv(os.path.join(output_data_folder, "traffic_fines_2.csv"), sep=";", index=False)
