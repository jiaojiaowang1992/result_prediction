...
model.compile(loss={'outcome_output':'binary_crossentropy'},metrics=['accuracy'], optimizer=opt)
...
for nr_events in range(1, max_len+1):
    ...
    X, y, case_ids = dataset_manager.generate_3d_data_for_prefix_length_org(dt_test, max_len, nr_events)
    #print("打印测试的X")
    #print(X.head(10))
    if X.shape[0] == 0:
        break
